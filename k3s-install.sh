...
download_and_verify() {
    ...
    setup_tmp
    get_release_version
    if test -f "/home/vagrant/k3s-cache/${VERSION_K3S}/sha256sum-${ARCH}.txt" ; then
        echo "Found cached k3s files: /home/vagrant/k3s-cache/${VERSION_K3S}/sha256sum-${ARCH}.txt"
        cp "/home/vagrant/k3s-cache/${VERSION_K3S}/sha256sum-${ARCH}.txt" ${TMP_HASH}
    else
        download_hash
    fi
    load_expected_hash
    ...
    if test -f "/home/vagrant/k3s-cache/${VERSION_K3S}/k3s${SUFFIX}" ; then
        echo "Found cached k3s files: /home/vagrant/k3s-cache/${VERSION_K3S}/k3s${SUFFIX}"
        cp "/home/vagrant/k3s-cache/${VERSION_K3S}/k3s${SUFFIX}" ${TMP_BIN}
    else
        download_binary
    fi
